...
export default {
  ...
  computed: {
    menu() {
      ...
      let items = pm_apply_filters("pm-project-menu", [
        {
          route: {
            name: "task_lists",
        ...
        },
        {
            ...
            name: this.__( "Payments", "wedevs-project-manager" ),
            count: '',
            class: 'payments pm-sm-col-12',
            order: 6
        }
      ]);
      ...
      this.moreMenu = [...items.slice(8, items.length)];
  ...
  methods: {
    ...
    setActiveMenu(item) {
      ...
      if (
        (item.route.name == "task_lists" &&
          (name == "single_list" || name == "task_lists_pagination")) ||
        (item.route.name == "discussions" &&
